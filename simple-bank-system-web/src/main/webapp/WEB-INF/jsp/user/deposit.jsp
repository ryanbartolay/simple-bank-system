...
<html>
<head>
<title>Deposit for account ${user.id}</title>
...
<body>
  ...
  <c:url var="depositUrl" value="/rest/transaction/deposit" />
  <div id="transactionMessage"></div>
  <form:form action="${depositUrl}" method="post"
    modelAttribute="deposit" id="depositForm">
    <form:input type="hidden" path="userId" value="${userId}" id="userId" />
    <form:input type="hidden" path="accountId" value="${accountId}" id="accountId" />
    <form:input type="hidden" path="transactionId" id="transactionId"
      value="${transactionId}" />
    <table>
      <tr>
        <td>Deposit amount</td>
        <td><input type="number" name="amount" id="amount"/> <span
          id="amountErrors" style="color: red;"></span></td>
      </tr>
      <tr>
        <td colspan="2" align="center"><button 
          name="transaction" value="CANCEL" id="submitCancelButton">Cancel</button>
          <button name="transaction" value="DEPOSIT"
          id="submitDepositButton">Deposit</button></td>
      </tr>
    </table>
  </form:form>
  <script type="text/javascript">
  $(function () {
	  $("#depositForm").submit(function(e){
          ...
          var action = $("#depositForm").attr("action");
          ...
          $.ajax({
            ...
            success: function (data, status) {
              if (data.status == "COMPLETED") {
                  $("#transactionMessage").append("<span style=\"color: green\">Deposit Completed</span>");
                  setTimeout(function () {
  ...
  function disableComponents(disable) {
    ...
    $("#submitDepositButton").attr("disabled", disable);
    if (!disable) {
      $("#amount").val("");
    } else {
      $("#amountErrors").empty();
